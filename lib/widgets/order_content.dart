...
class _OrderContentState extends State<OrderContent> {
  ...
  // Chờ xử lý: khách có thể hủy tự do
  // Đã xác nhận: có thể hủy (shop đồng ý) hoặc chuyển giao
  // Đang giao: không thể hủy, chỉ giao xong
  // Đã giao: có thể hoàn trả
  // Đã hủy / Hoàn trả: trạng thái cuối
  static const Map<String, List<String>> _validTransitions = {
    ...
    'Đã giao': ['Hoàn trả'],               // Chỉ hoàn trả sau khi giao
    'Đã hủy': [],                           // Trạng thái cuối
    'Hoàn trả': [],                         // Trạng thái cuối
  };

  static const _tabs = ['Tất cả', 'Chờ xử lý', 'Đã xác nhận', 'Đang giao', 'Đã giao', 'Đã hủy', 'Hoàn trả'];
  ...
  Widget build(BuildContext context) {
    ...
    return AdminSlidePanel(
      ...
      onClose: _closePanel,
      panelBody: _isPanelOpen ? _buildStatusPanelBody() : null,
      panelFooter: null,
  ...
  Widget _bulkBar(bool isDark, OrderProvider provider) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        ...['Đã xác nhận', 'Đang giao', 'Đã giao', 'Đã hủy', 'Hoàn trả'].map((s) => Padding(
    ...
    return InkWell(
      ...
      onTap: () async {
        final ok = await provider.bulkUpdateStatus(_selected.toList(), status,
          onStockChanged: () => Provider.of<ProductProvider>(context, listen: false).loadProducts(),
        );
        if (!mounted) return;
  ...
  Widget _actionMenu(Order o, bool isDark, OrderProvider provider) {
    final tp = isDark ? Colors.white : const Color(0xFF111827);
    final canCancel = (_validTransitions[o.status] ?? []).contains('Đã hủy');
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        ...
        if (canCancel) _popItem(Icons.cancel_rounded, 'Hủy đơn', 'cancel', const Color(0xFFEF4444)),
      ],
      onSelected: (v) {
        if (v == 'detail') setState(() { _showDetail = true; _detailOrder = o; });
        if (v == 'status') _openStatusPanel(o);
        if (v == 'cancel' && canCancel) _confirmCancel(o, provider);
      },
    );
  }
  ...
  Widget _buildStatusPanelBody() {
    ...
    final ts = isDark ? Colors.white.withValues(alpha: 0.5) : const Color(0xFF6B7280);
    final bdr = isDark ? Colors.white.withValues(alpha: 0.08) : const Color(0xFFE5E7EB);
    final o = _statusOrder;
    if (o == null) return const SizedBox.shrink();

    final validNext = _validTransitions[o.status] ?? [];
    final isTerminal = validNext.isEmpty;
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(
          color: _statusColor(o.status).withValues(alpha: 0.08),
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: _statusColor(o.status).withValues(alpha: 0.3)),
        ),
        child: Row(children: [
          Container(width: 8, height: 8, decoration: BoxDecoration(color: _statusColor(o.status), shape: BoxShape.circle)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Đơn ${o.code}', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: tp)),
            Text('Trạng thái hiện tại: ${o.status}', style: TextStyle(fontSize: 11, color: ts)),
          ])),
        ]),
      ),
      ...
      if (isTerminal) ...[
        ...
        Center(child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.check_circle_outline_rounded, size: 36, color: isDark ? Colors.white.withValues(alpha: 0.15) : const Color(0xFFD1D5DB)),
            const SizedBox(height: 8),
            Text('Đơn hàng đã ở trạng thái cuối', style: TextStyle(fontSize: 13, color: ts)),
          ]),
        )),
      ] else ...[
        Text('Chuyển sang:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: tp)),
        ...
        ...validNext.map((s) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 8), child: InkWell(
            ...
            child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              ...
              child: Row(children: [
                Icon(_statusIcon(s), size: 16, color: sc),
                const SizedBox(width: 10),
                Text(s, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: sc)),
                const Spacer(),
                Icon(Icons.arrow_forward_rounded, size: 14, color: sc),
              ]),
            ),
          ));
        }),
  ...
  IconData _statusIcon(String s) {
    switch (s) {
      case 'Chờ xử lý': return Icons.hourglass_top_rounded;
      case 'Đã xác nhận': return Icons.check_circle_rounded;
      case 'Đang giao': return Icons.local_shipping_rounded;
      case 'Đã giao': return Icons.task_alt_rounded;
      case 'Đã hủy': return Icons.cancel_rounded;
      case 'Hoàn trả': return Icons.assignment_return_rounded;
    ...
    return Column(children: [
      Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          const Spacer(),
          ElevatedButton.icon(onPressed: () => _openStatusPanel(o),
  ...
  Color _statusColor(String s) {
    switch (s) {
      ...
      case 'Hoàn trả': return const Color(0xFFF97316);
