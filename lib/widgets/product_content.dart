...
class _ProductContentState extends State<ProductContent> {
  ...
  Widget _buildPanelBody() {
    ...
    return Column(
      ...
      children: [
        ...
        _buildSectionTitle('Thông tin cơ bản', Icons.info_outline_rounded, isDarkMode),
        const SizedBox(height: 12),
        _buildTextField('Tên sản phẩm *', _nameController, isDarkMode),
        const SizedBox(height: 14),
        Row(
          children: [
            Expanded(child: _buildTextField('Mã SKU', _skuController, isDarkMode, hintText: 'VD: SP-001')),
            const SizedBox(width: 14),
            Expanded(child: _buildTextField('Thương hiệu', _brandController, isDarkMode, hintText: 'VD: Nike, Adidas')),
          ],
        ),
        const SizedBox(height: 14),
        _buildTextField('Mô tả sản phẩm', _descriptionController, isDarkMode, maxLines: 3, hintText: 'Mô tả chi tiết về sản phẩm...'),
        const SizedBox(height: 14),
        CloudinaryImagePicker(controller: _imageUrlController, isDarkMode: isDarkMode, label: 'Hình ảnh sản phẩm'),
        const SizedBox(height: 24),

        // ── Section 2: Danh mục & Giá ──
        _buildSectionTitle('Danh mục & Giá', Icons.sell_rounded, isDarkMode),
        const SizedBox(height: 12),
        _buildLabel('Danh mục *', isDarkMode),
        const SizedBox(height: 8),
        _buildCategoryDropdown(categories, setState, isDarkMode),
        const SizedBox(height: 14),
        Row(
          children: [
            Expanded(child: _buildTextField('Giá bán (đ) *', _priceController, isDarkMode, keyboardType: TextInputType.number, hintText: 'VD: 350000')),
            const SizedBox(width: 14),
            Expanded(child: _buildTextField('Giá khuyến mãi (đ)', _salePriceController, isDarkMode, keyboardType: TextInputType.number, hintText: 'Tùy chọn')),
          ],
        ),
        const SizedBox(height: 24),

        // ── Section 3: Kích thước & Màu sắc ──
        _buildSectionTitle('Kích thước & Màu sắc', Icons.straighten_rounded, isDarkMode),
        const SizedBox(height: 12),
        _buildLabel('Kích thước', isDarkMode),
        const SizedBox(height: 8),
        _buildSizeSelector(setState, isDarkMode),
        const SizedBox(height: 14),
        _buildLabel('Màu sắc', isDarkMode),
        const SizedBox(height: 8),
        _buildColorSelector(setState, isDarkMode),
        const SizedBox(height: 24),

        // ── Section 4: Kho hàng & Chi tiết ──
        _buildSectionTitle('Kho hàng & Chi tiết', Icons.inventory_2_rounded, isDarkMode),
        const SizedBox(height: 12),
        Row(
          children: [
            Expanded(child: _buildTextField('Tồn kho *', _stockController, isDarkMode, keyboardType: TextInputType.number, hintText: 'VD: 100')),
            const SizedBox(width: 14),
            Expanded(child: _buildTextField('Chất liệu', _materialController, isDarkMode, hintText: 'VD: Cotton, Polyester')),
          ],
        ),
        const SizedBox(height: 14),
        _buildStatusToggle(setState, isDarkMode),
      ],
    );
  }
    ...
    return Row(
      ...
      children: [
        TextButton(
          ...
          child: Text('Hủy', style: TextStyle(color: isDarkMode ? Colors.white.withValues(alpha: 0.6) : AppTheme.textLight)),
        ),
        ...
        ElevatedButton.icon(
          ...
          icon: Icon(_isEditMode ? Icons.save_rounded : Icons.add_rounded, color: Colors.white, size: 18),
          label: Text(_isEditMode ? 'Cập nhật' : 'Thêm sản phẩm', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AdminSlidePanel(
      isOpen: _isPanelOpen,
      title: _isEditMode ? 'Chỉnh sửa sản phẩm' : 'Thêm sản phẩm mới',
      onClose: _closePanel,
      panelBody: _isPanelOpen ? _buildPanelBody() : null,
      panelFooter: _isPanelOpen ? _buildPanelFooter() : null,
      child: Column(
        children: [
          // Header + filter bar
          Padding(
