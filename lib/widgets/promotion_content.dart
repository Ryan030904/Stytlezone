...
class _PromotionContentState extends State<PromotionContent> {
  ...
  String _panelTitle = 'Tạo khuyến mãi mới';
  Promotion? _editingPromo;

  final _codeCtrl = TextEditingController();
  final _promoNameCtrl = TextEditingController();
  ...
  void _openAddPanel() {
    _isEditMode = false;
    _panelTitle = 'Tạo khuyến mãi mới';
    _editingPromo = null;
    _codeCtrl.clear(); _promoNameCtrl.clear(); _descCtrl.clear();
    _valueCtrl.clear(); _minOrderCtrl.text = '0'; _maxUsesCtrl.text = '0';
    _discountType = DiscountType.percent;
    _promoStartDate = DateTime.now();
    _promoEndDate = DateTime.now().add(const Duration(days: 30));
    _promoIsActive = true;
    setState(() => _isPanelOpen = true);
  }

  void _openEditPanel(Promotion promo) {
    _isEditMode = true;
    _panelTitle = 'Sửa khuyến mãi';
    _editingPromo = promo;
    _codeCtrl.text = promo.code;
    _promoNameCtrl.text = promo.name;
    _descCtrl.text = promo.description;
    _valueCtrl.text = promo.discountValue.toStringAsFixed(0);
    _minOrderCtrl.text = promo.minOrderAmount.toStringAsFixed(0);
    _maxUsesCtrl.text = '${promo.maxUses}';
    _discountType = promo.discountType;
    _promoStartDate = promo.startDate;
    _promoEndDate = promo.endDate;
    _promoIsActive = promo.isActive;
    setState(() => _isPanelOpen = true);
  }
  ...
  Widget _buildPanelBody() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: field('Mã khuyến mãi', _codeCtrl, hint: 'VD: SALE50')),
        const SizedBox(width: 16),
        Expanded(child: field('Tên khuyến mãi', _promoNameCtrl, hint: 'VD: Giảm giá mùa hè')),
      ]),
      const SizedBox(height: 16),
      field('Mô tả', _descCtrl, hint: 'Mô tả ngắn (tuỳ chọn)', maxLines: 2),
      const SizedBox(height: 16),
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Loại giảm giá', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ts)),
          const SizedBox(height: 6),
          Container(padding: const EdgeInsets.symmetric(horizontal: 14),
            ...
            child: DropdownButtonHideUnderline(child: DropdownButton<DiscountType>(
              value: _discountType, isExpanded: true, dropdownColor: bg,
              style: TextStyle(fontSize: 13, color: tp),
              items: const [
                DropdownMenuItem(value: DiscountType.percent, child: Text('Phần trăm (%)')),
                DropdownMenuItem(value: DiscountType.fixed, child: Text('Cố định (VNĐ)')),
              ],
              onChanged: (v) => setState(() => _discountType = v!),
            ))),
        ])),
        const SizedBox(width: 16),
        Expanded(child: field('Giá trị giảm', _valueCtrl,
          hint: _discountType == DiscountType.percent ? 'VD: 20' : 'VD: 50000',
          type: TextInputType.number)),
      ]),
      const SizedBox(height: 16),
      Row(children: [
        Expanded(child: field('Đơn tối thiểu (VNĐ)', _minOrderCtrl, hint: '0 = không giới hạn', type: TextInputType.number)),
        const SizedBox(width: 16),
        Expanded(child: field('Số lần sử dụng tối đa', _maxUsesCtrl, hint: '0 = không giới hạn', type: TextInputType.number)),
      ]),
      const SizedBox(height: 16),
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Bắt đầu', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ts)),
          const SizedBox(height: 6),
          InkWell(borderRadius: BorderRadius.circular(10), onTap: () async {
            final picked = await showDatePicker(context: context,
              initialDate: _promoStartDate, firstDate: DateTime(2024), lastDate: DateTime(2030),
            ...
            if (picked != null) setState(() => _promoStartDate = picked);
            ...
            child: Row(children: [
              ...
              Text(_fmtDate(_promoStartDate), style: TextStyle(fontSize: 13, color: tp)),
            ]))),
        ])),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Kết thúc', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ts)),
          const SizedBox(height: 6),
          InkWell(borderRadius: BorderRadius.circular(10), onTap: () async {
            final picked = await showDatePicker(context: context,
              initialDate: _promoEndDate, firstDate: DateTime(2024), lastDate: DateTime(2030),
              builder: (ctx, child) => Theme(data: Theme.of(ctx).copyWith(
                colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF7C3AED), brightness: isDark ? Brightness.dark : Brightness.light)), child: child!));
            if (picked != null) setState(() => _promoEndDate = picked);
          }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
            decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(10), border: Border.all(color: bdr)),
            child: Row(children: [
              Icon(Icons.calendar_today_rounded, size: 16, color: ts), const SizedBox(width: 8),
              Text(_fmtDate(_promoEndDate), style: TextStyle(fontSize: 13, color: tp)),
            ]))),
        ])),
      ]),
      const SizedBox(height: 16),
      Row(children: [
        Text('Kích hoạt', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: tp)),
        const Spacer(),
        Switch(value: _promoIsActive, onChanged: (v) => setState(() => _promoIsActive = v), activeColor: const Color(0xFF7C3AED)),
      ]),
    ]);
  }
  ...
  Widget _buildPanelFooter() {
    ...
    return Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: ElevatedButton(onPressed: _savePromotion,
  ...
  Future<void> _savePromotion() async {
    ...
    final name = _promoNameCtrl.text.trim();
    if (code.isEmpty || name.isEmpty) {
      AppSnackBar.error(context, 'Vui lòng nhập mã và tên khuyến mãi');
      return;
    }
    final discountValue = double.tryParse(_valueCtrl.text) ?? 0;
    if (discountValue <= 0) {
    ...
    final promo = Promotion(
      ...
      usedCount: _editingPromo?.usedCount ?? 0,
      startDate: _promoStartDate,
      endDate: _promoEndDate,
      isActive: _promoIsActive,
      createdAt: _editingPromo?.createdAt ?? now,
      updatedAt: now,
    );
    _closePanel();
    final provider = Provider.of<PromotionProvider>(context, listen: false);
    final success = _isEditMode ? await provider.updatePromotion(promo) : await provider.createPromotion(promo);
    if (!mounted) return;
    if (success) {
      AppSnackBar.success(context, _isEditMode ? 'Cập nhật khuyến mãi thành công' : 'Tạo khuyến mãi thành công');
    } else {
