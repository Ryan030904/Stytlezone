...
class _ReportContentState extends State<ReportContent> {
  ...
  Future<void> _loadProductStats() async {
    ...
    // Tính sold từ đơn đã giao
    final soldMap = <String, int>{};
    try {
      final orderSnap = await _fs.collection('orders').get();
      for (final od in orderSnap.docs) {
        final data = od.data();
        final status = (data['status'] ?? '').toString();
        if (status == 'Đã giao' || status.toLowerCase() == 'delivered') {
          final items = data['items'] as List<dynamic>? ?? [];
          for (final item in items) {
            if (item is Map<String, dynamic>) {
              final pid = (item['productId'] ?? '').toString();
              final qty = (item['quantity'] as num?)?.toInt() ?? 0;
              if (pid.isNotEmpty) soldMap[pid] = (soldMap[pid] ?? 0) + qty;
            }
          }
        }
      }
    ...
    // top bán chạy — sort by sold (tính từ đơn đã giao)
