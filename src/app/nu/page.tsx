...
import { getFemaleProducts } from "@/lib/products";
import { getFemaleCategories } from "@/lib/categories";
...
const PRICE_RANGES = [
    ...
    { label: "15 — 50 triệu", min: 15000000, max: 50000000 },
            ...
            <FilterGroup title="Khoảng giá">
                {PRICE_RANGES.map((range, i) => (
                    <FilterCheckbox
                        ...
                        onChange={() => onSetPriceRange(selectedPriceRange === i ? null : i)}
                    />
                ))}
            </FilterGroup>
        ...
        <div ref={cardRef}>
            <Link
                ...
                onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow =
                        "0 20px 40px rgba(0,0,0,0.5), 0 0 20px rgba(139,92,246,0.08), 0 1px 0 rgba(255,255,255,0.06) inset";
                }}
                onMouseLeave={(e) => {
            ...
            >
                ...
                <div style={{ padding: "var(--space-lg)" }}>
                    <p style={{ fontSize: "0.72rem", color: "rgba(156,163,175,0.9)", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 500 }}>
                        {product.brandName || product.categoryName}
                    </p>
                    <h4 style={{
                        fontSize: "0.95rem", fontWeight: 600, color: "var(--text-primary)",
                        marginBottom: "var(--space-sm)", lineHeight: 1.45,
                    ...
                    <div style={{ display: "flex", alignItems: "baseline", gap: "var(--space-sm)" }}>
                        ...
                        {originalPrice && (
                            <span style={{ fontSize: "0.78rem", color: "rgba(107,114,128,0.7)", textDecoration: "line-through" }}>
...
function SkeletonCard() {
    return (
        <div style={{ borderRadius: "var(--radius-lg)", overflow: "hidden", background: "var(--bg-card)", border: "1px solid var(--border-color)" }}>
            <div style={{
                aspectRatio: "3/4",
...
function Pagination({
    ...
    const btnBase: React.CSSProperties = {
        minWidth: "40px", height: "40px", borderRadius: "var(--radius-md)",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: "0.85rem", fontWeight: 600, transition: "all 0.25s ease", cursor: "pointer",
    };

    return (
        <div ref={ref} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "var(--space-sm)", marginTop: "var(--space-3xl)" }}>
            <button
                disabled={currentPage === 1}
                onClick={() => onPageChange(currentPage - 1)}
                style={{
                    ...btnBase, border: "1px solid var(--border-color)",
            ...
            {pages.map((page, i) =>
                page === "..." ? (
                    <span key={`dots-${i}`} style={{ ...btnBase, color: "var(--text-muted)", cursor: "default" }}>…</span>
                ) : (
                    <button
                        key={page}
                        ...
                        style={{
                            ...btnBase,
                            border: page === currentPage ? "1px solid var(--color-accent)" : "1px solid var(--border-color)",
                            background: page === currentPage ? "var(--color-accent)" : "transparent",
                            color: page === currentPage ? "var(--color-white)" : "var(--text-secondary)",
                            boxShadow: page === currentPage ? "0 0 16px rgba(139,92,246,0.3)" : "none",
                        }}
                        onMouseEnter={(e) => {
                            if (page !== currentPage) {
...
export default function WomenPage() {
    ...
    useEffect(() => {
        if (!danhMucParam || categories.length === 0) return;
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (selectedCategories.length > 0) {
            ...
            result = result.filter((p) => expandedCategoryIds.has(p.categoryId));
        }
        ...
        switch (sortBy) {
            case "price-asc":
                result.sort((a, b) => {
                    const pa = a.salePrice > 0 && a.salePrice < a.price ? a.salePrice : a.price;
                    const pb = b.salePrice > 0 && b.salePrice < b.price ? b.salePrice : b.price;
                    return pa - pb;
                });
                break;
            case "price-desc":
        ...
        return result;
    }, [allProducts, categories, selectedCategories, selectedBrands, selectedPriceRange, selectedSizes, sortBy]);
    ...
    const toggleFilter = <T,>(arr: T[], value: T, setter: React.Dispatch<React.SetStateAction<T[]>>) => {
    ...
    const toggleCategory = (id: string) => {
        ...
        setSelectedCategories((prev) => {
            ...
            if (isParent) {
                // Toggle parent → toggle all its children
                ...
                if (isSelected) {
                    ...
                    // Deselect parent if any child is unchecked
                    if (cat.parentId) {
    ...
    return (
        <>
            <section ref={contentRef} id="products" className="section" style={{ paddingTop: "calc(var(--header-height) + var(--space-2xl))" }}>
                <div className="container">
                    ...
                    <div
                        style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: "var(--space-2xl)", alignItems: "start" }}
                        className="women-layout"
                    >
                        {/* Sidebar */}
                        <div className="filter-sidebar" style={{ position: "sticky", top: "calc(var(--header-height) + var(--space-lg))" }}>
                            <FilterSidebar
                                categories={categories} brands={brands}
                                selectedCategories={selectedCategories} selectedBrands={selectedBrands}
                                selectedPriceRange={selectedPriceRange} selectedSizes={selectedSizes}
                                onToggleCategory={toggleCategory}
                                onToggleBrand={(id) => toggleFilter(selectedBrands, id, setSelectedBrands)}
                                onSetPriceRange={(idx) => { setSelectedPriceRange(idx); setCurrentPage(1); window.scrollTo({ top: 0, behavior: "instant" }); }}
                                onToggleSize={(s) => toggleFilter(selectedSizes, s, setSelectedSizes)}
                                onClearAll={clearAllFilters} totalResults={filteredProducts.length}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-xl)", flexWrap: "wrap", gap: "var(--space-md)" }}>
                                <p style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>
                                    Hiển thị <strong style={{ color: "var(--text-primary)" }}>{paginatedProducts.length}</strong> / {filteredProducts.length} sản phẩm
                                </p>
                                <SortDropdown
                                    value={sortBy}
                            ...
                            {/* Product grid */}
                            {loading ? (
                                <div className="women-products-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--space-lg)" }}>
                                    {[...Array(6)].map((_, i) => <SkeletonCard key={i} />)}
                                </div>
                            ) : paginatedProducts.length === 0 ? (
                                <div style={{ textAlign: "center", padding: "var(--space-4xl) 0", color: "var(--text-muted)" }}>
                                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ margin: "0 auto var(--space-lg)", opacity: 0.4 }}>
                                        <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
                                    </svg>
                                    <p style={{ fontSize: "1rem", marginBottom: "var(--space-sm)" }}>Không tìm thấy sản phẩm phù hợp</p>
                                    <p style={{ fontSize: "0.85rem" }}>
                                        Hãy thử thay đổi bộ lọc hoặc{" "}
                                        <button onClick={clearAllFilters} style={{ color: "var(--color-accent)", fontWeight: 600, textDecoration: "underline", cursor: "pointer" }}>
                                            xóa tất cả bộ lọc
                                        </button>
                                    </p>
                                </div>
            ...
            {mobileFilterOpen && (
                <div style={{ position: "fixed", inset: 0, zIndex: 998, background: "rgba(0,0,0,0.6)" }} onClick={() => setMobileFilterOpen(false)} />
            )}
            ...
            <div
                ...
                style={{
                    ...
                    padding: "var(--space-2xl) var(--space-lg)", overflowY: "auto",
                }}
            >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-xl)" }}>
                    <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "var(--text-primary)" }}>Bộ lọc</h3>
                    <button onClick={() => setMobileFilterOpen(false)} style={{ color: "var(--text-secondary)", padding: "4px" }}>
                ...
                <FilterSidebar
                    ...
                    onToggleCategory={toggleCategory}
                    onToggleBrand={(id) => toggleFilter(selectedBrands, id, setSelectedBrands)}
                    onSetPriceRange={(idx) => { setSelectedPriceRange(idx); setCurrentPage(1); }}
                    onToggleSize={(s) => toggleFilter(selectedSizes, s, setSelectedSizes)}
                    onClearAll={clearAllFilters} totalResults={filteredProducts.length}
                />
            </div>
        ...
        @media (max-width: 1024px) {
          .women-products-grid { grid-template-columns: repeat(2, 1fr) !important; }
        }
        @media (max-width: 768px) {
          .women-layout { grid-template-columns: 1fr !important; }
          .filter-sidebar { display: none !important; }
          .mobile-filter-btn { display: inline-flex !important; margin-bottom: var(--space-lg); }
          .women-products-grid { grid-template-columns: repeat(2, 1fr) !important; gap: var(--space-md) !important; }
        }
      `}</style>
        </>
    );
