...
class PaymentService with BaseServiceMixin {
  ...
  Future<PaginatedResult<PaymentModel>> fetchPage({
    ...
    // Filter trên Firestore (server-side) để pagination chính xác
    if (status != null) {
      query = query.where('status', isEqualTo: EnumMapper.paymentStatus(status));
    }
    if (method != null) {
      query = query.where('method', isEqualTo: EnumMapper.paymentMethod(method));
    }
